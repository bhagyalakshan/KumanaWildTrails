...
const Taxi = () => {
  ...
  useEffect(() => {
    axios.get('http://localhost:8080/api/packages')
      .then(res => {
        const taxiPackages = res.data.filter(pkg =>
          pkg.type && pkg.type.trim().toLowerCase() === 'taxi'
        );
        setPackages(taxiPackages);
      })
      .catch(err => {
